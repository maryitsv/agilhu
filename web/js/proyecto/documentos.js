...
dataStoreDocxHU = new Ext.data.Store({
  ...
  dataStoreDocxHU.load();
	...
	tbar:[
		...
		{text:'Ocultar',id:'btnOcultar',iconCls:'sin_panel',scale:'large',handler:ocultarMostrar,tooltip:'Pulse aqui para ocultar el panel detallado de documentos'},'-',
...
function ocultarMostrar(){
	if(Ext.getCmp('btnOcultar').getText()=='Ocultar'){formDocumento.ownerCt.hide();
           formDocumento.ownerCt.ownerCt.doLayout();
           gridPanelListaDocumentos.doLayout();
	   Ext.getCmp('btnOcultar').setIconClass('con_panel');
	   Ext.getCmp('btnOcultar').setText('Mostrar');
	}
	else
	{
	var bot = Ext.getCmp('bottom-preview');
		
	bot.add(formDocumento);
    	bot.show();
    	bot.ownerCt.doLayout();
	Ext.getCmp('btnOcultar').setIconClass('sin_panel');
	
	Ext.getCmp('btnOcultar').setText('Ocultar');
      	}
...
/*INTEGRACION AL CONTENEDOR*/
	var panelContenedorDocumentos = new Ext.Panel({
            ...
            monitorResize:true,
            layout:'border',
            items: [gridPanelListaDocumentos,
                {
                id:'bottom-preview',
                layout:'fit',
                items:formDocumento,
		...
		{
                  url = URL_AGILHU+'pusuario.php/documentos/cargar?task=DESCARGAR&Identificador='+Ext.getCmp('idDocumento').getValue();
