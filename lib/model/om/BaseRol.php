...
abstract class BaseRol extends BaseObject  implements Persistent {
  ...
  const PEER = 'RolPeer';
	...
	/**
	 * Initializes internal state of BaseRol object.
	 * @see        applyDefaults()
	 */
	...
	public function setRolName($v)
	{
		...
		if ($this->rol_name !== $v) {
			$this->rol_name = $v;
			$this->modifiedColumns[] = RolPeer::ROL_NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = RolPeer::NUM_COLUMNS - RolPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RolPeer::ROL_NAME)) $criteria->add(RolPeer::ROL_NAME, $this->rol_name);
	 ...
	 * Otherwise if this Rol has previously been saved, it will retrieve
	 * related Usuarios from storage. If this Rol is new, it will return
		...
		if ($this->collUsuarios === null) {
			if ($this->isNew()) {
			   $this->collUsuarios = array();
				...
				$criteria->add(UsuarioPeer::USU_ROL_ID, $this->rol_id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseRol:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseRol::%s', $method));
    }
